#Final GUI with apply Homography matrix to cam 1
...
homography_matrix_cam1 = np.array([
    [ 9.71755665e-01, -3.39118016e-02,  1.53411716e+02],
...
def open_cameras():
    ...
    try:
        if not cameras_open:
            ...
            cam2 = Camera(camera_index=1, apply_homography=True, homography_matrix=homography_matrix_cam1)
...
def record_loop():
    try:
        ...
        out0 = cv2.VideoWriter('/home/lab902/Videos/V5.avi', fourcc, frame_rate, resolution)
        out1 = cv2.VideoWriter('/home/lab902/Videos/V6.avi', fourcc, frame_rate, resolution)
...
class Camera:
    def __init__(self, camera_index=0, apply_homography=False, homography_matrix=None):
        ...
        self.cam.start()
        self.apply_homography = apply_homography
        self.homography_matrix = homography_matrix

    def capture(self):
        frame = self.cam.capture_array()
        frame = cv2.resize(frame, resolution, interpolation=cv2.INTER_LINEAR)
        if self.apply_homography and self.homography_matrix is not None:
            frame = cv2.warpPerspective(frame, self.homography_matrix, resolution)
        return frame
